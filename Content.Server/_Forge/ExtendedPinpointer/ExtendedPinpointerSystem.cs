...
public sealed class ExtendedPinpointerSystem : SharedExtendedPinpointerSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ExtendedPinpointerComponent, ActivateInWorldEvent>(OnActivate);
        SubscribeLocalEvent<FTLCompletedEvent>(OnLocateTarget);
    }

    public override bool TogglePinpointer(EntityUid uid, ExtendedPinpointerComponent? pinpointer = null)
    {
    ...
    private void UpdateAppearance(EntityUid uid, ExtendedPinpointerComponent pinpointer, AppearanceComponent? appearance = null)
    {
    ...
    private void OnActivate(EntityUid uid, ExtendedPinpointerComponent component, ActivateInWorldEvent args)
    {
    ...
    private void OnLocateTarget(ref FTLCompletedEvent ev)
    {
        ...
        var query = EntityQueryEnumerator<ExtendedPinpointerComponent>();
    ...
    private void LocateTarget(EntityUid uid, ExtendedPinpointerComponent component)
    {
        // try to find target from whitelist
        if (component.IsActive && component.TargetId != null && component.Whitelist != null)
        {
            if (!EntityManager.ComponentFactory.TryGetRegistration(component.Whitelist, out var becomesStation))
            {
                DebugTools.Assert(false);
                return;
            }

            var target = FindTargetFromComponent(uid, becomesStation.Type, component.TargetId);
    ...
    /// </summary>
    private EntityUid? FindTargetFromComponent(EntityUid uid, Type whitelist, string id, TransformComponent? transform = null) //
    {
        ...
        foreach (var (otherUid, _) in EntityManager.GetAllComponents(whitelist))
        {
            ...
            if (!TryComp<BecomesStationComponent>(otherUid, out var becomesStation) || becomesStation.Id != id)
                continue;
    ...
    /// </summary>
    protected override void UpdateDirectionToTarget(EntityUid uid, ExtendedPinpointerComponent? pinpointer = null)
    {
        if (!Resolve(uid, ref pinpointer))
            return;
