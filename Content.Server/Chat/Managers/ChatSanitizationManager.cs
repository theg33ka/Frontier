...
public sealed class ChatSanitizationManager : IChatSanitizationManager
{
    ...
    [
        ...
        Entry("дрочит", "молится"),
        Entry("ебет", "молится"),
        Entry("ебёт", "молится"),
        Entry("трахает", "молится"),
        Entry("срёт", "молится"),
        Entry("срет", "молится"),
        Entry("ссыт", "молится"),
        Entry("обоссал", "помолился"),
        Entry("зигует", "бьёт себя по лицу"),
        Entry("зиганул", "бьёт себя по лицу"),
        Entry("кинул плотную", "бьёт себя по лицу"),
        Entry("кинул потную", "бьёт себя по лицу"),
        Entry("]=)", "chatsan-smiles"),
        Entry("--", "кажется не впечатлённым"),
        ...
        [NotNullWhen(true)] out string? emote)
    {
        ...
        foreach (var (r, emoteKey) in ShorthandToEmote) // Corvax-S-Localization
        {
    ...
    /// <summary>
    ///     Creates a regex that will match a shorthand.
    /// </summary>
    /// <param name="shorthand">The shorthand to match</param>
    /// <param name="emoteKey">The localization key for the emote</param>
    /// <returns>A tuple of the regex and the emote key</returns>
    /// <remarks>
    ///     This is a method so that we can have a nice, clean looking array of shorthands.
    ///     The regex is as follows:
    ///     - It can be preceded by a whitespace character, and must be followed by punctuation, whitespace, or the end of the line.
    ///       This is to prevent it from matching in the middle of a word.
    ///     - Or it can be at the start of the line, and must be followed by punctuation, whitespace, or the end of the line.
    ///       This is to prevent it from matching in the middle of a word.
    ///     - It is case-insensitive.
    ///     - It is matched from right to left, so that we can get the last emote in the message.
    /// </remarks>
