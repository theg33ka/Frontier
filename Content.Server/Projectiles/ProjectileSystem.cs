...
using Content.Shared._Mono; // Frontier
...
public sealed class ProjectileSystem : SharedProjectileSystem
{
    ...
    private void OnStartCollide(EntityUid uid, ProjectileComponent component, ref StartCollideEvent args)
    {
        ...
        if (component.RandomBlindChance > 0.0f &&
            _random.Prob(component.RandomBlindChance)) // Frontier - bb make you go blind
            TryBlind(target);
        ...
        while (query.MoveNext(out var uid, out var projectileComp, out var physicsComp, out var xform))
        {
            ...
            const int collisionMask =
                (int)(CollisionGroup.Impassable | CollisionGroup.BulletImpassable |
                      CollisionGroup
                          .Opaque); // Replaces CollisonGroup.MobMask with CollisionGroup.BulletImpassable - Mono
            ...
            if (hits.Count > 0)
            {
                ...
                if (attemptEv.Cancelled)
                {
                    ...
                    if (!Deleted(uid))
                        _transformSystem.SetLocalRotation(uid, currentVelocity.ToAngle() + new Angle(MathF.PI));
                ...
                var ev = new ProjectileHitEvent(
                    projectileComp.Damage * _damageableSystem.UniversalProjectileDamageModifier,
                    hitEntity,
                    projectileComp.Shooter);
                RaiseLocalEvent(uid, ref ev); // Raise on the projectile itself

                if (projectileComp.RandomBlindChance > 0.0f &&
                    _random.Prob(projectileComp.RandomBlindChance)) // Frontier
                    TryBlind(hitEntity);
                ...
                var modifiedDamage = _damageableSystem.TryChangeDamage(hitEntity,
                    ev.Damage,
                    projectileComp.IgnoreResistances,
                ...
                if (projectileComp.ProjectileSpent)
                {
                    if (!deleted)
                    {
                        _guns.PlayImpactSound(hitEntity,
                            modifiedDamage,
                            projectileComp.SoundHit,
                            projectileComp.ForceSound);
                    ...
                    _transformSystem.SetWorldPosition(uid, hitPosition);
                    ...
                    if (projectileComp.ImpactEffect != null)
                    {
                        ...
                        RaiseNetworkEvent(
                            new ImpactEffectEvent(projectileComp.ImpactEffect, GetNetCoordinates(xform.Coordinates)),
                            Filter.Pvs(xform.Coordinates, entityMan: EntityManager));
                    }
                }
            }
