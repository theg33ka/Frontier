...
/// <summary>
/// Unary coloration strategy for monochromatic skin tones
/// </summary>
[DataDefinition]
[Serializable, NetSerializable]
public sealed partial class MonochromaticSkinColoration : ISkinColorationStrategy
{
    [DataField]
    public float MinHue { get; private set; } = 0f;

    [DataField]
    public float MaxHue { get; private set; } = 0f;

    [DataField]
    public float MinSaturation { get; private set; } = 0f;

    [DataField]
    public float MaxSaturation { get; private set; } = 0f;

    [DataField]
    public float MinValue { get; private set; } = 0.05f;

    [DataField]
    public float MaxValue { get; private set; } = 0.15f;
    ...
    public Color FromUnary(float unary)
    {
        var tone = Math.Clamp(unary, 0f, 100f);
        var tNorm = tone / 100f;

        float hue = MinHue;
        float saturation = MinSaturation + tNorm * (MaxSaturation - MinSaturation);
    ...
    public float ToUnary(Color color)
    {
        ...
        float tone = (hsv.Z - MinValue) / (MaxValue - MinValue) * 100f;
